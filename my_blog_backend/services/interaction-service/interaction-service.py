...
class InteractionServerService:
    """
    互动服务业务逻辑类，负责处理点赞和评论相关的核心业务逻辑
    """
    ...
    def __init__(self, db_path: str, linkgateway_url: str = "http://localhost:8000"):
        ...
        self.interaction_service = InteractionService(db_path, linkgateway_url)
        self.logger = logger
        
        self.logger.info("互动管理服务初始化成功")
    ...
    def start(self) -> bool:
        ...
        """
        try:
            self.logger.info("互动管理服务启动成功")
            return True
        except Exception as e:
            self.logger.error(f"互动管理服务启动失败: {str(e)}")
            return False
    ...
    def stop(self) -> bool:
        ...
        """
        try:
            self.logger.info("互动管理服务停止成功")
            return True
        except Exception as e:
            self.logger.error(f"互动管理服务停止失败: {str(e)}")
            return False
    ...
    def get_formatted_api_routes(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for route in api_routes:
                ...
                # 格式化API路径
                path = APIStandard.format_api_path("business", "interaction-service", route.get("path"))
