...
def get_logger(name: str = "post-service", log_dir: Optional[str] = None) -> logging.Logger:
    """
    获取或创建日志实例
    ...
    if not log_dir:
        ...
        log_dir = os.path.join(project_root, "log", "services", "post-service")
    
    os.makedirs(log_dir, exist_ok=True)
    
    log_file = os.path.join(log_dir, "post-service")
