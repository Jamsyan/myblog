...
def get_logger(name: str = "user-server", log_dir: Optional[str] = None) -> logging.Logger:
    """
    获取或创建日志实例
    ...
    if not log_dir:
        ...
        log_dir = os.path.join(project_root, "log", "services", "user-server")
    
    os.makedirs(log_dir, exist_ok=True)
    
    log_file = os.path.join(log_dir, "user-server")
